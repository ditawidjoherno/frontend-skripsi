...
const TodoList = () => {
  ...
  const [isDissapear, setIsDissapear] = useState({});
  ...
  return (
    <div className="bg-white md:flex-row flex-col sm:ml-11 sm:mt-10 mt-4 rounded-2xl sm:h-[250px] h-[220px] sm:w-[650px] w-[300px]">
      ...
      <ul className="space-y-2 px-8 py-3 sm:text-base text-xs">
        {activeTodos.map((todo) => (
          <li
            key={todo.id}
            className={`flex items-center pl-4 h-10 ${isDone[todo.id] ? "bg-green-600" : "bg-red-600"
              } rounded-xl ${isDissapear[todo.id] ? "hidden" : ""}`}
          >
